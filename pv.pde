...
float circumRadius (pt A, pt B, pt C)     // radiusCircum(A,B,C): radius of circumcenter 
  ...
  s=(a+b+c)/2, 
  d=sqrt(s*(s-a)*(s-b)*(s-c)); //HERON'S FORMULA FOR AREA.
  return a*b*c/4/d; // USING TRIANGLE FORMULAE
  } 
...
pt pointOnArcThrough (pt A, pt B, pt C, float t) //WHAT IS THE METHOD USED FOR FINDING THE POINT ON THE ARC THROUGH A,B,C
   { 
...
float curvature(pt A, pt B, pt C) // computes 1 / radius of circumcircle to (A,B,C) //USES TRIANGLE LAWS
  {
  float a=d(B,C), b=d(C,A), c=d(A,B); // edge lengths
